...
public class Interceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request,
        ...
        if ((securityContext.getAuthentication() != null)
            ...
            c.setUserDetails((UserDetails) securityContext.getAuthentication()
                    .getPrincipal());
        } else {
            if (request.getUserPrincipal() != null) {
                AttributePrincipal principal = (org.jasig.cas.client.authentication.AttributePrincipal) request
                        .getUserPrincipal();
                // System.err.println("REMOVEME : Authenticated via CAS : " +
                // principal.getName());
                ...
                if (u == null) {
                    ...
                    u = userDAO.createUser(principal.getName(),
                            String.valueOf(principal.getAttributes().get(
                                    "firstname")), String.valueOf(principal
                                    .getAttributes().get("lastname")), String
                                    .valueOf(principal.getAttributes().get(
                                            "email")), new Md5PasswordEncoder()
                                    .encodePassword(StringUtils
                                            .generateRandomString(10, 50),
                                            principal.getName()), // password
                            new Md5PasswordEncoder().encodePassword(
                                    StringUtils.generateRandomString(10, 50),
                                    principal.getName()), // regkey
                            "ROLE_USER");
                    u.setActive(true);
                    updateUser = true;
                }
                if (!u.getEmailAddress().equals(
                        String.valueOf(principal.getAttributes().get("email")))) {
                    u.setEmailAddress(String.valueOf(principal.getAttributes()
                            .get("email")));
                    updateUser = true;
                }
                if (!u.getFirstName().equals(
                        String.valueOf(principal.getAttributes().get(
                                "firstname")))) {
                    u.setFirstName(String.valueOf(principal.getAttributes()
                            .get("firstname")));
                    updateUser = true;
                }
                if (!u.getLastName().equals(
                        String.valueOf(principal.getAttributes()
                                .get("lastname")))) {
                    u.setLastName(String.valueOf(principal.getAttributes().get(
                            "lastname")));
                    updateUser = true;
                }
                ...
                Authentication auth = new AnonymousAuthenticationToken(
                        u.getName(), principal, grantedAuth);
        ...
        if (c.getUser() != null) {
            ...
            // Intentionally using session.get to work around filtering in this
            // case.
            // the user may be the root user which has no portal id.
            // User rebind = (User)c.getHibernate().get(User.class,
            // c.getUser().getId());
            User rebind = userDAO.getUser(c.getUser().getId());
            if (rebind != null && c.getUserDetails() != null) {
                ((au.com.gaiaresources.bdrs.security.UserDetails) c
                        .getUserDetails()).setUser(rebind);
            }
        }
        ...
        if (menu == null || menu.size() < 1) {
            // write an error message so the user will know why they don't have
            // any menus
    ...
    private void configureApplicationMode(ModelAndView modelAndView) {
        if (appModeService.isMaintenanceModeActive()) {
            for (AbstractApplicationMode mode : appModeService
                    .getMaintenanceModeList()) {
    ...
    public void postHandle(HttpServletRequest request,
        ...
        if (modelAndView != null) {
            ...
            if (request.getParameter(Theme.DISABLE_THEME) != null) {
                requestContext.addMessage(propertyService
                        .getMessage("theme.disabled"));
            ...
            if (modelAndView.getView() instanceof RedirectView) {
                ...
                // RedirectView by default, exposes model attributes in the URL
                // querystring.
                // So, only add these model items if we aren't using a
                // redirectview. The items
                // will get added to the model eventually when the handler for
                // the non redirect
                // view occurs.
                String googleMapKey = gkService.getGoogleMapApiKey(request
                        .getServerName());
                if (googleMapKey == null) {
                    if (!request.getRequestURL().toString()
                            .startsWith("http://localhost")) {
                        // google maps always works with localhost so only log
                        // when NOT requested from localhost
                ...
                if (loggedInUser != null && loggedInUser.getId() != null) {
                    modelAndView.getModel().put(MV_USER_ID,
                            loggedInUser.getId().toString());
                    modelAndView.getModel().put(MV_USER_ROLE,
                            Role.getHighestRole(loggedInUser.getRoles()));
                    modelAndView.getModel().put(MV_USER_IS_ADMIN,
                ...
                BdrsPluginFacade facade = new BdrsPluginFacade(
                        requestContext.getHibernate(),
                        requestContext.getPortal(), request.getRequestURI(),
                ...
                if (theme != null) {
                    ...
                    if (theme.getId() != null) {
                        ...
                        ThemePage themePage = themeDAO
                                .getThemePage(theme.getId().intValue(),
                                        modelAndView.getViewName());
                        if (themePage != null) {
                            modelAndView.addObject(PARAM_PAGE_TITLE,
                                    themePage.getTitle());
                            modelAndView.addObject(PARAM_PAGE_DESCRIPTION,
                                    themePage.getDescription());
                        }
                    }
                }
            }
            ...
            if (request.getParameter("format") != null) {
                ...
                } else if (request.getParameter("format").equalsIgnoreCase(
                        "json")) {
                    modelAndView.setViewName("jsonDummyView");
                    response.setContentType("application/json");
                    response.getWriter().write(
                            parseJSON(modelAndView.getModel()).toString());
                    response.getWriter().flush();
                    response.getWriter().close();
                }
            }
